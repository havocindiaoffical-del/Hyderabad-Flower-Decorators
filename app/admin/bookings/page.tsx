                  ...
                  <tr key={b.id} className="transition-colors cursor-pointer" style={{ borderBottom: `1px solid ${borderColor}` }}
                    ...
                    <td className="py-3 px-4">
                      {b.image_share_urls?.length > 0 ? (
                        <span className="text-xs font-body text-sage font-medium">✓ {b.image_share_urls.length}</span>
                      ) : b.images?.length > 0 ? (
                        <span className="text-xs font-body" style={{ color: "#B8935F" }}>⏳ {b.images.length}</span>
                      ) : (
              ...
              <div className="sm:col-span-2">
                <label className="text-xs font-body mb-2 block" style={{ color: textSecondary }}>
                  ...
                  {selectedBooking.images?.length > 0 ? (
                    <span className="ml-1" style={{ color: "#5B7553" }}> — {selectedBooking.images.length} image(s) ✓</span>
                  ) : (
                    <span className="ml-1" style={{ color: textMuted }}> — none</span>
                  )}
                </label>

                {/* Images exist — show thumbnails + Access Photos button */}
                {selectedBooking.images?.length > 0 && (
                  <>
                      ...
                      <div className="rounded-xl p-4 mb-3" style={{ background: "rgba(91,117,83,0.05)", border: "1px solid rgba(91,117,83,0.15)" }}>
                        <div className="flex items-center justify-between flex-wrap gap-2">
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-xs font-body font-semibold" style={{ color: "#5B7553" }}>✓ Photos ready — view & download</p>
                          </div>
                    ...
                    {/* Individual image thumbnails served from Neon DB */}
                    {selectedBooking.image_share_urls?.length > 0 && (
                      <div className="flex flex-wrap gap-3 mb-3">
                        {selectedBooking.image_share_urls.map((url, i) => (
                          <a key={i} href={url} target="_blank" rel="noopener noreferrer">
                            <div className="w-24 h-24 rounded-lg overflow-hidden border" style={{ background: hoverBg, borderColor }}>
                    ...
                    {selectedBooking.image_share_urls?.length > 0 && (
                      <div className="flex items-center gap-2 mb-3">
                        <button onClick={() => { navigator.clipboard.writeText(selectedBooking.image_share_urls!.join("\n")); setShareLinkCopied(true); setTimeout(() => setShareLinkCopied(false), 2000); }}
                          ...
                          {shareLinkCopied ? "✓ Copied!" : "Copy All Image URLs"}
                        </button>
                        <span className="text-xs font-body" style={{ color: textMuted }}>{selectedBooking.image_share_urls.length} links</span>
                      </div>
                    )}
                  </>
                )}
