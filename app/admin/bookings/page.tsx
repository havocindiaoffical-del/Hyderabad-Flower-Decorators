...
import {
  ...
  ChevronLeft, ChevronRight, Phone, Mail, MapPin, Ticket, Undo2, Download, FileText, Image as ImageIcon
} from "lucide-react";
...
export default function AdminBookings() {
  ...
  const handleDownloadZip = async () => {
    ...
    try {
      ...
      // Create customer details TXT file
      const eventLabel = selectedBooking.event_type.replace(/-/g, " ");
      const detailsContent = [
        "=" .repeat(50),
        "HYDERABAD FLOWER DECORATORS — BOOKING DETAILS",
        "=" .repeat(50),
        "",
        `Ticket ID    : ${selectedBooking.ticket_id}`,
        `Customer Name: ${selectedBooking.full_name}`,
        ...
        selectedBooking.google_maps_link ? `Google Maps  : ${selectedBooking.google_maps_link}` : "",
        ...
        selectedBooking.admin_notes ? `Admin Notes  : ${selectedBooking.admin_notes}` : "",
        "",
        ...
        `Images Included: ${selectedBooking.images.length}`,
        "",
        "=" .repeat(50),
        "Generated by Hyderabad Flower Decorators Admin Panel",
        "=" .repeat(50),
      ...
      const totalImages = selectedBooking.images.length;
      for (let i = 0; i < selectedBooking.images.length; i++) {
        const url = selectedBooking.images[i];
        setDownloadProgress(`Downloading image ${i + 1} of ${totalImages}...`);
        ...
        try {
          ...
          const hasExt = /\.(jpg|jpeg|png|webp)$/i.test(urlFileName);
          const fileName = hasExt ? urlFileName : `reference_image_${i + 1}.${ext}`;

          zip.file(`images/${fileName}`, blob);
        } catch {
          // Skip this image if download fails
          zip.file(`images/failed_image_${i + 1}.txt`, `Failed to download image from: ${url}`);
        }
      }
      ...
      // Create a safe filename from customer name
      const safeName = selectedBooking.full_name
        .replace(/[^a-zA-Z0-9\s]/g, "")
        .replace(/\s+/g, "_")
        .trim();

      const zipFileName = `${safeName}_${selectedBooking.ticket_id}.zip`;
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ borderBottom: `1px solid ${borderColor}` }}>
                  ...
                  <th className="text-left py-3 px-4 font-heading font-medium" style={{ color: textSecondary }}>Images</th>
                    ...
                    <td className="py-3 px-4">
                      {booking.images && booking.images.length > 0 ? (
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs font-body" style={{ color: textPrimary }}>{booking.images.length}</span>
                        </div>
                      ) : (
